...
if(isset($_POST['derivbuy'])){
    ...
    if($_POST['AmountInNaira'] >= $total_deriv_min_amount){
        $amount_in_naira = $_POST['AmountInNaira'];
    }else{
        die(header("location:./deriv-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairalength=true"));
    }
    
    if(empty($_POST['AmountInNaira'])){
        die(header("location:./deriv-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairacredential=true"));
    }
    ...
    if(empty($_POST['AccountDetails'])){;
        die(header("location:./deriv-buy?purchaseFailedinvalidAccountDetails=true&reason=invalid_AccountDetailscredential=true"));
    	die();
    	$error = true;
    }else{ 
	...
	if(!$error){
        ...
        if($fetch_order_status=="open"){
            die(header("location:./deriv-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        //}elseif($receipt=="0"){
            //die(header("location:./deriv-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        }else{
        $reg1 = "INSERT INTO buy_order(order_id , user_id , dollar_type , amount_in_naira , account_details , time , date) 
        VALUES(:order_id , :user_id , :dollar_type, :amount_in_naira , :account_details , :time , :date)";
    ...
    $message .= "<p style='font-size:14px;margin:0 0 6px 0;'><span style='font-weight:bold;display:inline-block;min-width:150px'>Order type</span> Deriv USD Purchase</p>";
    ...
    $message .= '</tbody>
	        ...
	        if ($mailsend) {
                die(header("location:./deriv-buy?orderid=$order_id&amount=$amount_in_naira"));
                $con = null;
            }
        }
        
	}else{
        die(header("location:./deriv-buy?purchaseFailedunknownerror=true&reason=Failed_unknownerror=true"));
	}
