...
if(isset($_POST['netellerbuy'])){
    ...
    $neteller_min_quantity = "20"; 
    $total_neteller_min_amount = $neteller_buy * $neteller_min_quantity;
    
    }
    
    if($_POST['AmountInNaira'] >= $total_neteller_min_amount){
        $amount_in_naira = $_POST['AmountInNaira'];
    }else{
        die(header("location:./neteller-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairalength=true"));
    }
    
    if(empty($_POST['AmountInNaira'])){
        die(header("location:./neteller-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairacredential=true"));
    }
    ...
    if(empty($_POST['netellerid'])){
        die(header("location:./neteller-buy?purchaseFailedinvalidnetellerid=true&reason=invalid_netelleridcredential=true"));
    }else{
        $netellerid = $_POST['netellerid'];
    }
    
    if(empty($_POST['AccountDetails'])){;
        die(header("location:./neteller-buy?purchaseFailedinvalidAccountDetails=true&reason=invalid_AccountDetailscredential=true"));
    	die();
    	$error = true;
    }
    
    if(ctype_alpha($_POST['AccountDetails'])){
	...
	if(!$error){
        ...
        if($fetch_order_status=="open"){
            die(header("location:./neteller-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        //}elseif($receipt=="0"){
            //die(header("location:./neteller-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        }else{
        $reg1 = "INSERT INTO buy_order(order_id , user_id , dollar_type , amount_in_naira , account_details , address , time , date) 
        VALUES(:order_id , :user_id , :dollar_type, :amount_in_naira , :account_details , :netellerid , :time , :date)";
        ...
        $stmt->bindParam(":netellerid",$netellerid);
    ...
    $message .= '</tbody>
	        ...
	        if ($mailsend) {
                die(header("location:./neteller-buy?orderid=$order_id&amount=$amount_in_naira"));
                $con = null;
            }
        }
        
	}else{
        die(header("location:./neteller-buy?purchaseFailedunknownerror=true&reason=Failed_unknownerror=true"));
	}
