...
if(isset($_POST['bitcoinsell'])){
    ...
    $bitcoin_min_quantity = "20"; 
    //$total_bitcoin_min_amount = $bitcoin_sell * $bitcoin_min_quantity;
    
    }
    
    if($_POST['AmountInUSD'] >= $bitcoin_min_quantity){
        $amount_in_usd = $_POST['AmountInUSD'];
    }else{
        die(header("location:./bitcoin-sell?sellFailedinvalidAmountInUSD=true&reason=invalid_AmountInUSDlength=true"));
    }
    
    if(empty($_POST['AmountInUSD'])){
        die(header("location:./bitcoin-sell?sellFailedinvalidAmountInUSD=true&reason=invalid_AmountInUSDcredential=true"));
    }
	...
	if(!$error){
        ...
        if($fetch_order_status=="open"){
            die(header("location:./bitcoin-sell?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        //}elseif($receipt=="0"){
            //die(header("location:./bitcoin-sell?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        }else{
    ...
    $message .= "<p style='font-size:14px;margin:0 0 6px 0;'><span style='font-weight:bold;display:inline-block;min-width:150px'>Order type</span> Bitcoin Sell</p>";
    ...
    $message .= '<p style="font-size:14px;margin:0;padding:10px;border:solid 1px #ddd;font-weight:bold;"><span style="display:block;font-size:13px;font-weight:normal;">1. Visit your order in dashboard.</span></p>
          ...
          <p style="font-size:14px;margin:0;padding:10px;border:solid 1px #ddd;font-weight:bold;"><span style="display:block;font-size:13px;font-weight:normal;">3. Proceed to withdrawal using the stated bitcoin address in order page.</span></p>
      ...
      <tr>
        <td colspan="2" style="font-size:14px;padding:50px 15px 0 15px;">
          <strong style="display:block;margin:0 0 10px 0;">Regards,<br>Support<br><a href="https://sahmax.com">https://www.sahmax.com/</a></strong>  
			...
			$subj = "$$amount_in_usd Bitcoin Sell Order Has Been Placed";
    ...
    $message .= '</tbody>
	        ...
	        if ($mailsend) {
                die(header("location:./bitcoin-sell?orderid=$order_id&amount=$amount_in_naira"));
                $con = null;
            }
        }
        
	}else{
        die(header("location:./bitcoin-sell?sellFailedunknownerror=true&reason=Failed_unknownerror=true"));
	}
