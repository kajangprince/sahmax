...
if(isset($_POST['bitcoinbuy'])){
    ...
    $bitcoin_min_quantity = "50"; 
    $total_bitcoin_min_amount = $bitcoin_buy * $bitcoin_min_quantity;
    
    }
    
    if($_POST['AmountInNaira'] >= $total_bitcoin_min_amount){
        $amount_in_naira = $_POST['AmountInNaira'];
    }else{
        die(header("location:./bitcoin-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairalength=true"));
    }
    
    if(empty($_POST['AmountInNaira'])){
        die(header("location:./bitcoin-buy?purchaseFailedinvalidAmountInNaira=true&reason=invalid_AmountInNairacredential=true"));
    }
    
    if(empty($_POST['address'])){
        die(header("location:./bitcoin-buy?purchaseFailedinvalidAddress=true&reason=invalid_Addresscredential=true"));
    }else{
    ...
    if(empty($_POST['AccountDetails'])){;
        die(header("location:./bitcoin-buy?purchaseFailedinvalidAccountDetails=true&reason=invalid_AccountDetailscredential=true"));
    	die();
    	$error = true;
    }
	...
	if(!$error){
        ...
        if($fetch_order_status=="open"){
            die(header("location:./bitcoin-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        //}elseif($receipt=="0"){
            //die(header("location:./bitcoin-buy?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        }else{
        $reg1 = "INSERT INTO buy_order(order_id , user_id , dollar_type , amount_in_naira , account_details , address , time , date) 
        VALUES(:order_id , :user_id , :dollar_type, :amount_in_naira , :account_details , :address , :time , :date)";
    ...
    $message .= "<p style='font-size:14px;margin:0 0 6px 0;'><span style='font-weight:bold;display:inline-block;min-width:150px'>Order type</span> Bitcoin Purchase</p>";
      ...
      <tr>
        <td colspan="2" style="font-size:14px;padding:50px 15px 0 15px;">
          <strong style="display:block;margin:0 0 10px 0;">Regards,<br>Support<br><a href="https://sahmax.com">https://www.sahmax.com/</a></strong>  
			...
			$subj = "Your $$amount_in_dollar worth of Bitcoin Order Has Been Placed";
    ...
    $message .= '</tbody>
			...
			$subj = "$$amount_in_dollar worth of Bitcoin Order Has Been Placed";
			$mailsend=mail("$adminemail","$subj","$message","$ehead");
	        
	        if ($mailsend) {
                die(header("location:./bitcoin-buy?orderid=$order_id&amount=$amount_in_naira"));
                $con = null;
            }
        }
        
	}else{
        die(header("location:./bitcoin-buy?purchaseFailedunknownerror=true&reason=Failed_unknownerror=true"));
	}
