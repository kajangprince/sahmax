...
if(isset($_POST['derivsell'])){
    ...
    $deriv_min_quantity = "5"; 
    //$total_deriv_min_amount = $deriv_sell * $deriv_min_quantity;
    
    }
    
    if($_POST['AmountInUSD'] >= $deriv_min_quantity){
        $amount_in_usd = $_POST['AmountInUSD'];
    }else{
        die(header("location:./deriv-sell?sellFailedinvalidAmountInUSD=true&reason=invalid_AmountInUSDlength=true"));
    }
    
    if(empty($_POST['AmountInUSD'])){
        die(header("location:./deriv-sell?sellFailedinvalidAmountInUSD=true&reason=invalid_AmountInUSDcredential=true"));
    }
	...
	if(!$error){
        ...
        if($fetch_order_status=="open"){
            die(header("location:./deriv-sell?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        //}elseif($receipt=="0"){
            //die(header("location:./deriv-sell?purchaseFailedOrderexist=true&reason=completeactiveorders=true"));
        }else{
    ...
    $message .= "<p style='font-size:14px;margin:0 0 6px 0;'><span style='font-weight:bold;display:inline-block;min-width:150px'>Order type</span> Deriv USD Withdrawal</p>";
    ...
    $message .= '<p style="font-size:14px;margin:0;padding:10px;border:solid 1px #ddd;font-weight:bold;"><span style="display:block;font-size:13px;font-weight:normal;">1. Visit your order in dashboard.</span></p>
          ...
          <p style="font-size:14px;margin:0;padding:10px;border:solid 1px #ddd;font-weight:bold;"><span style="display:block;font-size:13px;font-weight:normal;">3. Proceed to withdrawal using the stated payment agent account details in order page.</span></p>
      ...
      <tr>
        <td colspan="2" style="font-size:14px;padding:50px 15px 0 15px;">
          <strong style="display:block;margin:0 0 10px 0;">Regards,<br>Support<br><a href="https://sahmax.com">https://www.sahmax.com/</a></strong>  
			...
			$subj = "Your $amount_in_usd Deriv USD Withdrawal Order Has Been Placed";
    ...
    $message .= '</tbody>
			...
			$subj = "$amount_in_usd Deriv USD Withdrawal Order Has Been Placed";
			$mailsend=mail("$adminemail","$subj","$message","$ehead");
	        
	        if ($mailsend) {
                die(header("location:./deriv-sell?orderid=$order_id&amount=$amount_in_naira"));
                $con = null;
            }
        }
        
	}else{
        die(header("location:./deriv-sell?sellFailedunknownerror=true&reason=Failed_unknownerror=true"));
	}
